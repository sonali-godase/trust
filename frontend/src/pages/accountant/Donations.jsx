...
const Donations = () => {
  ...
  const fetchDonations = async () => {
    try {
      ...
      const nonPending = (res.data.data || []).filter(d => d.status !== 'PENDING_VERIFICATION');
      setDonations(nonPending);
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
  ...
  const handleExport = () => {
    ...
    exportToCSV(exportData, 'accountant_donations_export');
  };
  ...
  if (loading) return <div className="h-64 flex items-center justify-center"><div className="w-8 h-8 border-4 border-blue-500 rounded-full border-t-transparent animate-spin"></div></div>;
  ...
  return (
    <div className="w-full space-y-6 text-gray-800 pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} className="bg-gradient-to-br from-blue-500 to-indigo-600 p-6 rounded-2xl text-white shadow-lg shadow-blue-500/20 relative overflow-hidden group">
        ...
        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            ...
            <div>
              ...
              <select value={filterBranch} onChange={(e) => setFilterBranch(e.target.value)} className="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-blue-500">
