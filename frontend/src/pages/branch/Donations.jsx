...
const Donations = () => {
  ...
  const handleExport = () => {
    ...
    exportToCSV(exportData, 'branch_donations_export');
  };
    ...
    <div className="w-full space-y-6 text-gray-800 pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={handleExport} className="flex items-center gap-2 px-4 py-2.5 bg-blue-900 hover:bg-blue-800 text-white rounded-xl text-sm font-black transition-colors shadow-md">
            <FiDownload /> Export
          </button>
      ...
      {showFilters && (
        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
          ...
          <div className="flex justify-end mt-4">
            <button onClick={() => { setFilterYear(''); setFilterMonth(''); setFilterStatus(''); setFilterMinAmount(''); setFilterMaxAmount(''); }} className="text-sm font-bold text-gray-500 hover:text-gray-700">Clear Filters</button>
          </div>
        </motion.div>
      )}
